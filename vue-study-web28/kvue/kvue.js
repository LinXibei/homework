...
class Compile {
  ...
  update(node, exp, dir) {
    ...
    // if (typeof this.$vm[exp] === 'function') { 我自己写的
    //   fn && fn(node, this.$vm[exp].bind(this.$vm))
    // } else if (dir === 'model') {
    //   fn && fn(node, this.$vm[exp], exp, this.$vm) 
    // } else {
  ...
  compileElement(node) {
    Array.from(node.attributes).forEach((attr) => {
      ...
      if (this.isDir(attrName)) {
        ...
        this.eventHandle(node, exp, eventName)
        // this[eventName] && this[eventName](node, exp)  我自己写的
      }
    });
  }
  ...
  //   node.addEventListener('click', val, false) 
  // }
  // // k-model处理函数
  // model(node, exp) {
